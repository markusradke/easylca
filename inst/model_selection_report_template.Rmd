...
[Red indicates **models with an estimated class size below `r n_min`** for at least one class, **models that were not replicated** (2x best LL with different starting values), models with **boundary value estimates**, and **models with non-significant VLMRT (or adjusted VLMRT)**.]{style="color: #c40d20"}

```{r echo = F}
create_modeloverview_table(data, n_min)
```
