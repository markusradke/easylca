...
test_that("is all discrete indicators <= 10 levels works", {
  random_testdata <- data.frame(var1 = seq(11) %>% as.integer(),
                         id = 'id')
  expect_error(assert_data_format(random_testdata, 'id'),
...
test_that("is all indicators are either double or integer", {
  random_testdata <- data.frame(var1 = 'character',
                         var2 = 'character',
                         id = 'id')
  expect_error(assert_data_format(random_testdata, 'id'),
               'Please make sure all indicators are coded as either double or integer.\nProblem caused by: var1, var2')

  random_testdata <- data.frame(var1 = c(1,2) %>% as.integer(),
                         var2 = c(1., 2.1) %>% as.double(),
                         id = 'id')
  expect_no_condition(assert_data_format(random_testdata, 'id'))
})

test_that("report generation creates file in the current wd directory",{
  random_testdata_only_discrete <- data.frame(var1 = seq(10) %>% as.integer(),
                                       id = 'id')
  generate_data_diagnosis_report(random_testdata_only_discrete, 'id', 'testreport')
  is_file <- file.exists('data_diagnosis_testreport.html')
  expect_true(is_file)
  if(is_file){file.remove('data_diagnosis_testreport.html')}

  random_testdata_only_continuous <- data.frame(var1 = seq(10),
                                        id = 'id')
  generate_data_diagnosis_report(random_testdata_only_continuous, 'id', 'testreport')
  is_file <- file.exists('data_diagnosis_testreport.html')
  expect_true(is_file)
  if(is_file){file.remove('data_diagnosis_testreport.html')}

  random_testdata_both_types <- data.frame(var1 = seq(10) %>% as.integer(),
                                    var2 = seq(10),
                                    id = 'id')
  generate_data_diagnosis_report(random_testdata_both_types, 'id', 'testreport')
  is_file <- file.exists('data_diagnosis_testreport.html')
  expect_true(is_file)
  if(is_file){file.remove('data_diagnosis_testreport.html')}
})
