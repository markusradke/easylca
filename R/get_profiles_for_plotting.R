get_profiles_for_plotting <- function(
  model,
  settings,
  show_significance = FALSE
) {
  profile_types <- get_profile_types(settings)
  combined_profiles <- get_model_estimated_profiles(
    model,
    profile_types,
    show_significance
  )
  class_prevalences <- get_model_estimated_class_prevalences(model)
  profiles <- suppressMessages(dplyr::inner_join(
    combined_profiles,
    class_prevalences
  )) %>%
...
get_model_estimated_profiles <- function(
  model,
  profile_types,
  show_significance
) {
  if (length(profile_types$continuous) > 0) {
    continuous <- get_continuous_profiles(
      model,
      profile_types,
      show_significance
    )
  ...
  if (length(profile_types$categorical) > 0) {
    categoricals <- get_categorical_profiles(
      model,
  ...
  if (length(profile_types$nominal) > 0) {
    nominals <- get_nominal_profiles(
      model,
      profile_types$nominal,
      show_significance
    )
  } else {
    nominals <- c()
...
get_categorical_profiles <- function(
  model,
  categorical,
  binary,
  show_significance = FALSE
  ...
  categoricals <- model$parameters$probability.scale %>%
    dplyr::mutate(
      ...
      plotgroup = ifelse(.data$item %in% binary, 'binary', 'discrete'),
...
get_nominal_profiles <- function(model, nominal, show_significance = FALSE) {
  ...
  nominals <- calculate_probabilities_from_logits(
    nominals_logit,
    show_significance
  )
  nominals
...
calculate_probabilities_from_logits <- function(
  logit,
  show_significance = FALSE
) {
...
get_continuous_profiles <- function(
  model,
  profile_types,
  show_significance = FALSE
) {
  ...
  means <- correct_negbin_poisson_means(
    means,
    profile_types$negbin,
    profile_types$poisson
  )
  errors <- get_errors_from_profiles(continuous, means)
  errors <- correct_negbin_errors(
    means,
    errors,
    continuous,
    profile_types$negbin
  )
  errors <- correct_poisson_errors(
    means,
    errors,
    continuous,
    profile_types$poisson
...
get_errors_from_profiles <- function(profiles, means) {
  suppressMessages(
    profiles %>%
      dplyr::filter(
        .data$param == 'Variances' | .data$param == 'Dispersion'
      ) %>%
      dplyr::left_join(
        means %>% dplyr::select('item', 'class', 'mean' = 'est')
      ) %>%
      dplyr::mutate(
        sd = sqrt(.data$est),
...
correct_negbin_errors <- function(means, errors, continuous, negbin) {
  errors_dispersion <- suppressMessages(
    continuous %>%
      ...
      dplyr::select('class', 'item', 'disp' = 'est') %>%
      dplyr::inner_join(means) %>%
      dplyr::inner_join(errors) %>% # V(negbin) = mean + disp * mean**2
      dplyr::mutate(
        upper = .data$est + sqrt(.data$est + (.data$est**2) / .data$disp),
        lower = .data$est - sqrt(.data$est + (.data$est**2) / .data$disp)
      ) %>%
      dplyr::select('upper', 'lower', 'class', 'item')
  )

  errors %>% dplyr::filter(!.data$item %in% negbin) %>% rbind(errors_dispersion)
}
...
correct_poisson_errors <- function(means, errors, continuous, poisson) {
  errors_poisson <- means %>%
    ...
    dplyr::mutate(
      upper = .data$est + sqrt(.data$est), # V(poi) = mean
      lower = .data$est - sqrt(.data$est)
    ) %>%
    dplyr::select('upper', 'lower', 'class', 'item')

  errors %>% dplyr::filter(!.data$item %in% poisson) %>% rbind(errors_poisson)
}
...
calculate_ypos_for_pzero <- function(continuous) {
  upperclass <- suppressMessages(
    continuous %>%
      dplyr::mutate(
        upper = ifelse(is.na(.data$upper), .data$est, .data$upper)
      ) %>% # for poisson
  ...
  classrange <- suppressMessages(
    continuous %>%
      dplyr::mutate(
        lower = ifelse(is.na(.data$lower), .data$est, .data$lower)
      ) %>% # for poisson
      dplyr::group_by(.data$item) %>%
  ...
  classrange %>%
    dplyr::mutate(
      paramrange = .data$uppermax - .data$lowermin,
...
rename_items <- function(profiles, profile_types) {
  profiles %>%
    dplyr::mutate(
      item = ifelse(
        .data$item %in% profile_types$negbin,
        paste0(.data$item, '\n(negbin)'),
        .data$item
      ),
      item = ifelse(
        .data$item %in% profile_types$poisson,
        paste0(.data$item, '\n(poisson)'),
        .data$item
      )
    )
