...
#'
#' @examples
#' # generate report (will output a file)
...
assert_data_format <- function(frame, idcol){
  ...
  types <- sapply(frame %>% dplyr::select(-dplyr::all_of(idcol)), class)
  discrete <- types[types == 'integer'] %>% names()
  continuous <- types[types == 'numeric'] %>% names()

  wrong_types <- setdiff(colnames(frame %>% dplyr::select(-dplyr::all_of(idcol))),
  ...
  only_discretes <- frame %>%
    dplyr::select(dplyr::all_of(discrete))
