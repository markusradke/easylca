plot_class_prevalences <- function(model) {
  ...
  ggplot2::ggplot(
    class_counts_formatted,
    ggplot2::aes(x = .data$count, y = .data$class, fill = .data$class)
  ) +
    ggplot2::geom_col() +
    ggplot2::geom_vline(xintercept = 100, color = 'black', linewidth = 1) +
    ggplot2::annotate(
      'text',
      x = 100,
      y = nrow(class_counts_formatted),
      label = '100',
      color = 'white',
      size = 5,
      hjust = -0.1
    ...
    ggplot2::geom_text(
      ggplot2::aes(label = .data$proportion),
      size = 5,
      color = 'black',
      hjust = -0.1
    ) +
...
create_mean_assignement_probabilities_table <- function(model) {
  prediction_probabilites <- model$savedata %>%
    dplyr::mutate(
      cprob = apply(
        dplyr::select(., dplyr::starts_with('CPROB')),
        1,
        max,
        na.rm = TRUE
      )
  ...
  class_colors <- discrete_colors_for_classes[
    mean_assignement_probabilites$class
  ] # internal from package
...
plot_kruskal_profiles <- function(model) {
  ...
  .get_kruskal_chisquares <- function() {
    data.frame(item = items) %>%
      ...
      dplyr::mutate(
        ...
        chisquare = purrr::map_dbl(
          .data$item,
          .get_kruskal_chisquare_from_prediction_for_item
        ),
        p = purrr::map_chr(.data$item, .get_kruskal_p_from_prediction_for_item)
      ) %>%
  ...
  suppressWarnings(
    ggplot2::ggplot(
      chisquares,
      ggplot2::aes(
        y = stats::reorder(.data$item, .data$chisquare),
        x = .data$chisquare
      )
    ) +
      ggplot2::geom_bar(stat = 'identity', fill = 'grey') +
      ggplot2::geom_text(
        ggplot2::aes(label = .data$p),
        hjust = -0.1,
        size = 6,
        color = 'black'
      ) +
      ggplot2::scale_x_continuous(
        limits = c(0, max_chi_square * 1.5),
        expand = c(0, 0.05),
        position = 'top'
      ) +
      ggplot2::labs(
        subtitle = 'Kruskal-Wallis Chi square',
        y = '',
...
plot_continuous_profiles <- function(
  profiles,
  ncol_plot = 2,
  ...
  plot <- ggplot2::ggplot(
    average_means,
    ggplot2::aes(x = as.factor(.data$class), y = .data$est, color = .data$class)
  ) +
    ...
    ggplot2::geom_text(
      ggplot2::aes(y = .data$yposinflation, label = .data$pzero),
      vjust = 0.2,
      size = 3,
      color = 'black'
    ) +
    ...
    ggplot2::geom_hline(
      ggplot2::aes(yintercept = .data$average_mean),
      color = 'grey70',
      linewidth = 0.6,
      linetype = 'twodash'
    ) +
    ggplot2::geom_text(
      ggplot2::aes(y = .data$average_mean),
      label = 'mean',
      x = 0,
      hjust = -1,
      vjust = 1.05,
...
plot_binary_profiles <- function(profiles, show_significance = FALSE) {
  ...
  relative_freqs <- profiles %>%
    ...
    dplyr::mutate(
      share_label = paste0(round(.data$share_of_2, 2), .data$significance)
    ) %>%
    dplyr::select('class', 'item', 'share_of_2', 'share_label')

  plot <- ggplot2::ggplot(
    relative_freqs,
    ggplot2::aes(x = .data$class, y = .data$item, fill = .data$class)
  ) +
    ggplot2::geom_tile(
      ggplot2::aes(alpha = .data$share_of_2),
      color = "white",
      show.legend = FALSE
    ) +
    ggplot2::geom_text(
    ...
    ggplot2::scale_fill_manual(values = class_colors) +
    ggplot2::theme_minimal() +
    ggplot2::labs(y = '', subtitle = 'P(level 2 | class)', x = '') +
    ggplot2::theme(
      axis.text.y = ggplot2::element_text(size = 14, face = 'bold'),
      axis.text.x = ggplot2::element_text(
        size = 14,
...
plot_discrete_profiles <- function(profiles, show_significance = FALSE) {
  profiles <- dplyr::filter(profiles, .data$plotgroup == 'discrete') %>%
    dplyr::mutate(
      label = sprintf('%.0f%%%s', round(100 * .data$est), .data$significance),
  ...
  plot <- ggplot2::ggplot(
    data = profiles,
    ggplot2::aes(
      x = forcats::fct_inorder(as.character(.data$level)),
    ...
    ggplot2::geom_text(
      ggplot2::aes(label = .data$label, color = .data$label_color),
      hjust = 0,
      vjust = 0.5,
      nudge_x = -0.25
    ) +
    ...
    ggplot2::labs(subtitle = 'P(level | class)', x = 'level', y = '') +
    ggplot2::theme_minimal() +
    ggplot2::theme(
      panel.grid = ggplot2::element_blank(),
      legend.position = 'none',
